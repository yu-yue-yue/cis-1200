...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[newH][newW]; // swap coordinates

        for (int row = 0; row < newH; row++) {
            for (int col = 0; col < newW; col++) {
                if ((row < borderWidth) || (col < borderWidth) ||
                    (row >= newH - borderWidth) || (col >= newW - borderWidth)) {
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                tgt[row][col] = new Pixel(
                        255 - src[row][col].getRed(),
                        255 - src[row][col].getGreen(),
                        255 - src[row][col].getBlue()); // invert coordinates
            }
        }
        ...
        for (int r = 0; r < h; r++) {
            for (int c = 0; c < w; c++) {
                ...
                int average = Math.round(
                        (p.getRed() + p.getGreen() + p.getBlue()) / 3.0f
                );
                tgt[r][c] = new Pixel(average, average, average);
            }
        }
        return new PixelPicture(tgt);
    }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                tgt[row][col] = new Pixel(
                (int) (Math.round(src[row][col].getRed() * rfactor)),
                (int) (Math.round(src[row][col].getGreen() * gfactor)),
                (int) (Math.round(src[row][col].getBlue() * bfactor))
                ); 
            }
        }
    ...
    public static PixelPicture alphaBlend(
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                int[] comps1 = src1[row][col].getComponents();
                int[] comps2 = src2[row][col].getComponents();
                int[] tgtcomps = new int[3];
                for (int i = 0; i < tgtcomps.length; i++) {
                    tgtcomps[i] = weightedAverage(alpha, 
                    comps1[i], comps2[i]);
                }
                tgt[row][col] = new Pixel(tgtcomps); // 
            }
        }
