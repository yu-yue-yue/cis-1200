...
public class TweetParser {
     ...
     * commas.
     *
     * @param csvLine   - a line extracted from a FileLineIterator
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (csvColumn >= columns.length) {
            return null;
        }
        return columns[csvColumn]; // Complete this method.
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        if (br != null) {
            FileLineIterator text = new FileLineIterator(br);
            while (text.hasNext()) {
                rows.add(text.next());
            }

            for (String row : rows) {
                String maybe = extractColumn(row, tweetColumn);
                if (maybe != null && !maybe.equals("")) {
                    tweets.add(maybe);
                }
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<String> sentences = tweetSplit(t);
        for (String sentence : sentences) {
            List<String> cleaned = parseAndCleanSentence(sentence);
            if (!cleaned.isEmpty()) {
                words.add(cleaned);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String tweet: tweets) {
            words.addAll(parseAndCleanTweet(tweet));
        }
